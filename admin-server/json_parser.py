...
def parse_and_store(
    ...
    if not focus_summary and isinstance(session.get("activity_summary"), dict):
        focus_summary = session["activity_summary"].get("application_focus_seconds")
    ...
    try:
        with connection.cursor() as cur:
            ...
            # 4. Application focus (from focus_summary or computed from timeline events)
            if not focus_summary and counts["focus_timeline"] > 0:
                focus_tracker: dict[str, datetime] = {}
                for event in events:
                    etype = event.get("event_type", "")
                    proc = event.get("data", {}).get("process_name", "")
                    ts_str = event.get("timestamp")
                    if not proc or not ts_str:
                        continue
                    try:
                        ts = datetime.fromisoformat(ts_str.replace("Z", "+00:00"))
                    except Exception:
                        continue
                    if etype == "application_focused":
                        focus_tracker[proc] = ts
                    elif etype == "application_unfocused" and proc in focus_tracker:
                        delta = (ts - focus_tracker.pop(proc)).total_seconds()
                        if delta > 0:
                            focus_summary[proc] = round(focus_summary.get(proc, 0.0) + delta, 2)
            ...
            # 6. Session activity summary & productive_seconds calculation
            active_seconds = int(duration_seconds - desktop_seconds) if duration_seconds else None
            productive_total = 0.0
            if focus_summary:
                cur.execute("SELECT process_name, is_productive FROM application_catalog;")
                cat_map = {row[0]: row[1] for row in cur.fetchall()}
                for p_name, secs in focus_summary.items():
                    if p_name == "Desktop":
                        continue
                    is_prod = cat_map.get(p_name)
                    if is_prod is True or is_prod is None:
                        productive_total += secs
            productive_seconds = int(round(productive_total)) if focus_summary else 0
